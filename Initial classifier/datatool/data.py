...
class GraphField(data.Field):
    def preprocess(self, x):
        entities, slocs, bertlocs = x
        return (entities.strip(), slocs.strip(), bertlocs.strip())
...
class DocDataset(data.Dataset):
    def __init__(self, path, text_field, order_field, graph_field,
        ...
        examples = []
        path, einspath, orderpath, bertlocpath = path
        with open(path, 'r', encoding=encoding) as f, open(einspath, 'r') as fe, open(orderpath, 'r') as fo, open(bertlocpath, 'r') as fb:
            for line, lineeins, linesins, linebins in zip(f, fe, fo, fb):
                line = line.strip()
                order = list(range(line.count('<eos>') + 1))

                if 'train' in path:
                    if len(order) > 1:
                        examples.append(data.Example.fromlist([line, order, (lineeins, linesins, linebins)], fields))
                else:
                    examples.append(data.Example.fromlist([line, order, (lineeins, linesins, linebins)], fields))
...
class MyBatch(Batch):
    def __init__(self, allsentences, orders, doc_len, ewords, elocs, slocs,blocs=None, dataset=None,
                 device=None):
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                for ex in minibatch:
                    ...
                    for sandvalue in sg.split():
                        ...
                        value = float(value)
                        sent_newlocs.append((target[int(s_before)], target[int(s_after)], value))
                ...
                yield MyBatch(allsentences, orders, doc_len, ewords, elocs, slocs, blocs,
